...
export function getImageStorageProvider():ImageStorageProvider{return{
  async upload(file,options){
    ...
    const result=await new Promise<{secure_url:string;public_id:string}>((resolve,reject)=>{
      const stream=cloudinary.uploader.upload_stream({folder:`carxsailor/vehicles/${options.folder}`,resource_type:"image",allowed_formats:["jpg","jpeg","png","webp"],transformation:[{width:2000,height:2000,crop:"limit",quality:"auto"}],timeout:60000,use_filename:false,unique_filename:true,overwrite:false},(error,value)=>error||!value?reject(error??new Error("Cloudinary upload failed.")):resolve(value));
      stream.end(bytes);
    });
...
export function validateVehicleImage(file:File){const allowed=new Set(["image/jpeg","image/png","image/webp"]);if(!allowed.has(file.type))throw new Error("Only JPEG, PNG and WebP images are allowed.");if(file.size===0)throw new Error("Images must not be empty.");if(file.size>8*1024*1024)throw new Error("Images must be 8 MB or smaller.");return file}
