...
export function VehicleCard({vehicle,match,directDetails=false}:{vehicle:Vehicle;match?:number;directDetails?:boolean}){
  return <VehicleCardMotion>
    ...
    <div className="p-5">
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row"><div><p className="text-xs font-bold uppercase tracking-wider text-[#718079]">{vehicle.year} / {vehicle.bodyType}</p><h3 className="mt-1 text-lg font-extrabold"><Link href={`/cars/${vehicle.slug}`}>{vehicle.make} {vehicle.model}</Link></h3></div><b className="text-lg">{money(vehicle.price)}</b></div>
