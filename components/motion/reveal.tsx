...
export function Reveal({children,className="",family="rise",delay=0,hero=false}:{children:React.ReactNode;className?:string;family?:Family;delay?:number;hero?:boolean}){
  ...
  useEffect(()=>{if(!inView||seen.current)return;seen.current=true;const distance=desktop?revealMotion.desktopDistance:revealMotion.mobileDistance;const movement=reduced||family==="fade"?{}:family==="image"?{clipPath:["inset(0 100% 0 0 round 16px)","inset(0 0% 0 0 round 16px)"],scale:[desktop?1.085:1.04,1]}:family==="panel"?{clipPath:["inset(0% 0% 70% 0% round 24px)","inset(0% 0% round 24px)"],scale:[desktop?.94:.97,1]}:family==="left"||family==="right"?{x:[(family==="left"?-1:1)*(desktop?distance:12),0]}:{y:[distance,0]};const control=animate(scope.current,{opacity:[reduced?.8:0,1],...movement},{duration:reduced?.16:hero?timing.hero:desktop?timing.reveal:.9,ease:revealEase,delay:reduced?0:delay});return()=>control.complete()},[inView,animate,scope,reduced,desktop,family,delay,hero]);
...
export function TextReveal({lines,as:Tag="h2",className="",hero=false}:{lines:string[];as?:"h1"|"h2"|"h3";className?:string;hero?:boolean}){
  ...
  useEffect(()=>{if(!inView||played.current)return;played.current=true;const controls=Array.from(scope.current.querySelectorAll(".text-reveal-line")).map((node,i)=>animate(node as HTMLElement,reduced?{opacity:[.8,1]}:{y:["125%","0%"],rotate:[3,0],opacity:[0,1]},{duration:reduced?.15:hero?1.2:1.05,ease:revealEase,delay:reduced?0:(hero?.12:0)+i*revealMotion.lineStagger}));return()=>controls.forEach(control=>control.complete())},[inView,animate,scope,hero,reduced]);
